...
| 2026-05-16 | Avatar upload on Edit Profile | Premium-only. Camera FAB wired to file input (accept=image/*). Client-side optimize via `optimizeImage()` → POST `/api/profile/avatar`. R2 key `avatars/{userId}/{ulid}.webp`, old avatar deleted on re-upload. `users.imageKey` column stores R2 key; `users.image` kept for Google OAuth URL. Render priority: imageKey (signed URL) > image (Google) > initials. TopBar server component queries DB for imageKey to show fresh avatar everywhere. Remove button resets to Google photo or initials (DELETE endpoint, no premium gate) |
