...
- [x] Edit Profile (`/profile/edit`) — avatar upload (Premium, 2MB limit, client-side WebP optimize via `optimizeImage()`, R2 storage at `users/{userId}/avatar/{ulid}.webp`, signed read URLs, remove button), accent color picker (6 colors), display name (≤30 chars), email (read-only with verified badge), bio (≤160 chars), **delete account (wired)** — danger-zone button → confirm bottom sheet (store-subscription reminder, deleting state, inline error) → `DELETE /api/account` → `signOut` to `/login`. API: `POST /api/profile/avatar`, `DELETE /api/profile/avatar`, `DELETE /api/account`
...
- [x] App install / download prompt — mobile-only bottom-sheet (`install-app-prompt.tsx`) mounted on Home, shown once (localStorage `pwa-install-dismissed`, shared across both variants), never on desktop. **iOS (native app live):** a "Dailymood มีแอปบน iOS แล้ว" card — app icon + official Apple **"Download on the App Store"** badge (locale-aware EN/TH SVG on R2 `guide/app-store-badge-{en,th}.svg`, kept unmodified per Apple guidelines, uploaded via `scripts/upload-app-store-badge.ts`) linking to the App Store (`apps.apple.com/th/app/dailymood-mood-journal/id6778759803`, hardcoded `APP_STORE_URL` const) + a quiet "ไว้ทีหลัง" dismiss. The standalone-PWA guard is intentionally dropped for iOS so users who already added the web app to their home screen are still nudged to the native app. The old iOS 5-step add-to-home-screen carousel is retired (its `guide/ios-step-*.webp` assets are now unused). **Android (no native app yet):** keeps the add-to-home-screen guide — 3-step carousel (`guide/step-{1,2,3}.webp`), still skipped when already installed as a PWA (`display-mode: standalone`). Analytics (GA, prod-only): `trackAppPromptShown(ios|android)` on show + `trackAppDownloadClick(ios)` on badge tap.
...
- [x] Privacy page (`/privacy`) — hero card (purple gradient, shield icon, ENCRYPTED/NO DATA SELLING/NO ADS badges), TL;DR summary (4 items with icons), 7 numbered sections covering data collection, AI, images, third-party services, retention, rights, contact. i18n TH/EN. Linked from Settings + Pricing footer. **v1.1 (mobile-app launch):** scope = web + iOS/Android apps; discloses Google Analytics honestly (old "NO TRACKING/no third-party analytics" claim was false), push-notification device tokens (Expo), optional location (Google Maps), voice = on-device transcription (no audio stored), broadened AI feature list, IAP billing (App Store/Play + RevenueCat — we never see card data), in-app account deletion (web Stripe sub auto-cancelled; store sub must be cancelled by user), marketing one-click unsubscribe. **v1.1.1 (iOS App Privacy alignment):** Google Analytics scoped explicitly to **web only** (mobile ships no GA SDK; backend never forwards mobile events to GA — verified gtag is window-guarded + loaded only in layout.tsx + imported only in client components) so the policy matches an iOS App Privacy that declares no analytics; AI section now discloses that **photo analysis (AI Vision) sends the photo to Gemini**, not just notes. **Paper Desk reskin**: hero → clipped gradient folder (`PAClip` + chunky shadow); TL;DR → washi-taped white `.pa-sheet` (`--w-ink*`, `--w-rule` dividers); numbered sections stay loose long-form text on the desk; back button → `.pa-icon-btn`.
- [x] Support page (`/support`) — **public, no login** (App Store Connect "Support URL" + Play Console contact requirement; the in-app feedback form is login-gated so it can't serve as the store support URL). Mirrors privacy/terms (Paper Desk: teal clipped hero folder + floating 💬, mint-washi contact card, numbered FAQ sections, `.pa-icon-btn` back). Contact card: `contact@dailymood.me` (mailto) + "reply within 24 hours". 6 FAQ (TH/EN, `support` i18n namespace): cancel/manage subscription (App Store/Play vs web Stripe), restore purchases, forgot password / Google-only set-password, delete account, 14-day trial billing, Free vs Pro. Footer links to privacy/terms. Wired into Profile → About + Settings → About (the previously-dead "Help center" row now hrefs here) + desktop site footer. Component `support-page.tsx`.
- [x] Terms page (`/terms`) — hero card (peach gradient, handshake icon, effective date), TL;DR summary (4 items: age, conduct, billing, medical disclaimer), 7 numbered sections covering accounts, acceptable use, premium, content ownership, liability, changes, contact. i18n TH/EN. Linked from Settings + Pricing footer. **v1.1 (effective 2026-06-11, mobile-app launch):** terms cover web + iOS/Android apps; Pro billing = Stripe (web) or in-app purchase (App Store/Google Play, manage/cancel in store settings); refunds follow purchase channel's policy; 14-day trial terms (one per account, no card, auto-revert); in-app account deletion. **Paper Desk reskin**: same as Privacy — clipped gradient hero folder, washi-taped white TL;DR sheet, loose sections, `.pa-icon-btn` back.
...
- [x] Subscription Management (`/profile/subscription`) — 3 states: Free (trial CTA + Free vs Pro cards), Trial (countdown card + subscribe CTA), Paid Pro (dark card + billing portal + cancel). DB: `stripeSubscriptionId`, `currentPeriodEnd`, `cancelAtPeriodEnd`, `planInterval`, `trialActivatedAt`, `trialEndsAt` on `users` table. API: `GET /api/subscription`. **Paper Desk reskin** (`subscription-shell.tsx`): feature/usage/Free cards → white `.pa-sheet` (`--w-ink*`); the dark Pro card became a plum folder (`.pa-tab purple` + `PAClip`); trial-status + premium-upsell + trial-banner gradient cards kept (white text) with chunky shadows + a clip; canceling/portal-error/trial-expired notices → tinted `.pa-sheet`; back button → `.pa-icon-btn`. Generic `BottomSheet` cancel + `TrialConfirmSheet` left themed. **Cross-platform cancel routing:** the Stripe billing-portal/cancel UI shows only when `premiumSource === "stripe"` (not merely `hasStripeCustomer` — an IAP subscriber can carry a leftover Stripe customer id); IAP subs show "Manage or cancel in App Store / Google Play" (per `iapSource`) with no web cancel. Enforced server-side: `POST /api/stripe/portal` rejects with `manage_in_store` unless `premium_source === "stripe"`. `GET /api/subscription` now also returns `premiumSource`. (iOS↔Android cross-cancel is impossible by OS design; the app deep-links to the right store via `iapSource`.)
- [x] Account deletion — `DELETE /api/account` (Bearer/session): permanent. One `DELETE FROM users` cascades all user-owned tables (entries, mood types, events, device tokens, refresh tokens, AI caches, feedback, achievements, bookmarks); explicitly cleans `verification_tokens` (email-keyed), R2 images (profile + entry photos, best-effort), and cancels an active Stripe subscription (IAP subs: user cancels in store; our rows drop via cascade). Returns `{ok:true}`; app signs out immediately. Required by Play/App Store before publish.
