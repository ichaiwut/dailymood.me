...
- [x] iOS app download prompt — **iOS-only** bottom-sheet (`install-app-prompt.tsx`) mounted on Home, shown once. A "Dailymood มีแอปบน iOS แล้ว" card: app icon + official Apple **"Download on the App Store"** badge (locale-aware EN/TH SVG on R2 `guide/app-store-badge-{en,th}.svg`, kept unmodified per Apple guidelines, uploaded via `scripts/upload-app-store-badge.ts`) linking to the App Store (`apps.apple.com/th/app/dailymood-mood-journal/id6778759803`, hardcoded `APP_STORE_URL`) + a quiet "ไว้ทีหลัง" dismiss. Gate = `detectIOS()` only (iPhone/iPad incl. iPad desktop-mode); Android/desktop/everyone else see nothing. Dismiss flag = **new** key `localStorage["ios-app-prompt-dismissed"]` (not the old `pwa-install-dismissed`) so users who dismissed the retired add-to-home-screen guide still get shown this once. Analytics (GA, prod-only): `trackAppPromptShown("ios")` on show + `trackAppDownloadClick("ios")` on badge tap. **The PWA "Add to Home Screen" guide was fully removed** (both the Android 3-step carousel and the iOS 5-step one) — no native Android app exists yet, so non-iOS users get no prompt; the old `guide/step-*.webp` + `guide/ios-step-*.webp` R2 assets are now unused.
